...
func TestNewTufAutoupdater(t *testing.T) {
	...
	_, err := NewTufAutoupdater("https://example.com", binaryName, testRootDir, http.DefaultClient, s)
	...
	exposedRootDir := LocalTufDirectory(testRootDir, binaryName)
...
func TestExecute(t *testing.T) {
	...
	for _, binary := range binaries {
		...
		t.Run(fmt.Sprintf("TestRun: %s", binary), func(t *testing.T) {
			...
			autoupdater, err := NewTufAutoupdater(metadataServerUrl, binary, testRootDir, http.DefaultClient, s)
			require.NoError(t, err, "could not initialize new TUF autoupdater")
...
func Test_storeError(t *testing.T) {
	...
	autoupdater, err := NewTufAutoupdater(testMetadataServer.URL, binaryName, testRootDir, http.DefaultClient, setupStorage(t))
	require.NoError(t, err, "could not initialize new TUF autoupdater")
	...
	err = autoupdater.store.ForEach(func(k, _ []byte) error {
		// Confirm we saved error with key in correct format
		parts := strings.Split(string(k), "/")
		require.Equal(t, 2, len(parts), "malformed error key")

		// Confirm error is saved under correct binary
		require.Equal(t, binaryName, parts[0])

		// Confirm error is saved with reasonable timestamp
		ts, err := strconv.ParseInt(parts[1], 10, 64)
		require.NoError(t, err, "invalid timestamp in key: %s", parts[1])
...
func Test_cleanUpOldErrors(t *testing.T) {
	...
	require.NoError(t, autoupdater.store.Set([]byte(fmt.Sprintf("launcher/%d", oneHourAgo)), []byte("{}")), "could not set recent timestamp for test")

	// Add some old timestamps
	eightDaysAgo := time.Now().Add(-8 * 24 * time.Hour).Unix()
	require.NoError(t, autoupdater.store.Set([]byte(fmt.Sprintf("launcher/%d", eightDaysAgo)), []byte("{}")), "could not set old timestamp for test")
	twoWeeksAgo := time.Now().Add(-14 * 24 * time.Hour).Unix()
	require.NoError(t, autoupdater.store.Set([]byte(fmt.Sprintf("launcher/%d", twoWeeksAgo)), []byte("{}")), "could not set old timestamp for test")
	...
	require.NoError(t, err, "could not iterate over keys")
	require.Equal(t, 3, keyCountBeforeCleanup, "did not correctly seed errors in bucket")
