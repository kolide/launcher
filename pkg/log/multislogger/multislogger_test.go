...
func TestMultiSlogger(t *testing.T) {
	...
	multislogger.AddReplaceHandler("system", slog.NewJSONHandler(&systemLogBuf, &slog.HandlerOptions{Level: slog.LevelInfo}), SystemLogMatcher)
	multislogger.AddReplaceHandler("shipper", slog.NewJSONHandler(&shipperBuf, &slog.HandlerOptions{Level: slog.LevelInfo}))
	multislogger.AddReplaceHandler("debug", slog.NewJSONHandler(&debugLogBuf, &slog.HandlerOptions{Level: slog.LevelDebug}))
	...
	// replace shipper with a debug level handler
	multislogger.AddReplaceHandler("shipper", slog.NewJSONHandler(&shipperBuf, &slog.HandlerOptions{Level: slog.LevelDebug}))
	multislogger.Logger.Debug("debug_msg")
	require.Contains(t, debugLogBuf.String(), "debug_msg", "should be in debug log since it's debug level")
	require.Contains(t, shipperBuf.String(), "debug_msg", "should now be in shipper log since it's new handler was set to debug level")
	require.Empty(t, systemLogBuf.String(), "should not be in system log since there was no system_log=true attr")
	clearBufsFn()
