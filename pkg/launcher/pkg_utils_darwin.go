...
var darwinIdentifierWhitelistRegex = regexp.MustCompile(`[^a-zA-Z0-9-_]+`)
...
func ServiceName(identifier string) string {
	...
	sanitizedServiceName := darwinIdentifierWhitelistRegex.ReplaceAllString(identifier, "-")

	return fmt.Sprintf("com.%s.launcher", sanitizedServiceName)
}
