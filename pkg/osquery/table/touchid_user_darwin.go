...
func (t *touchIDUserConfigTable) generate(ctx context.Context, queryContext table.QueryContext) ([]map[string]string, error) {
	...
	for _, constraint := range q.Constraints {
		...
		configOutput, err := tablehelpers.Exec(ctx, t.slogger, 15, allowedcmd.Bioutil, []string{"-r"}, false, tablehelpers.WithUid(u.Uid))
		if err != nil {
		...
		countOut, err := tablehelpers.Exec(ctx, t.slogger, 15, allowedcmd.Bioutil, []string{"-c"}, false, tablehelpers.WithUid(u.Uid))
		if err != nil {
			t.slogger.Log(ctx, slog.LevelDebug,
