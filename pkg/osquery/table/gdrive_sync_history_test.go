...
func TestGDriveSyncHistoryInfo(t *testing.T) { //nolint:paralleltest // We need to update package-level vars in this test
	...
	require.NoError(t, err)
	_, err = db.ExecContext(t.Context(), `CREATE TABLE IF NOT EXISTS local_entry (inode TEXT, filename TEXT, modified TEXT, size TEXT, checksum TEXT);`)
	require.NoError(t, err)
	_, err = db.ExecContext(t.Context(), `INSERT INTO local_entry (inode, filename, modified, size, checksum) VALUES ("", "testfile", "", "", "");`)
	require.NoError(t, err)
	_, err = db.ExecContext(t.Context(), `CREATE TABLE IF NOT EXISTS cloud_entry (inode TEXT, filename TEXT, modified TEXT, size TEXT, checksum TEXT);`)
	require.NoError(t, err)
	_, err = db.ExecContext(t.Context(), `INSERT INTO cloud_entry (inode, filename, modified, size, checksum) VALUES ("", "testfile", "", "", "");`)
	require.NoError(t, err)
