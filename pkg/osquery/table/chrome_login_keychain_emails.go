...
func ChromeLoginKeychainEmails(client *osquery.ExtensionManagerClient, logger log.Logger) *table.Plugin {
	c := &ChromeLoginKeychainEmailsTable{
		client: client,
		logger: logger,
	}
	...
	return table.NewPlugin("kolide_chrome_login_keychain_emails", columns, c.generate)
}

type ChromeLoginKeychainEmailsTable struct {
	client *osquery.ExtensionManagerClient
	logger log.Logger
}

func (c *ChromeLoginKeychainEmailsTable) generateForPath(ctx context.Context, path string, username string) ([]map[string]string, error) {
	dir, err := ioutil.TempDir("", "kolide_chrome_login_keychain_emails")
	if err != nil {
		return nil, err
	}
...
func (c *ChromeLoginKeychainEmailsTable) generate(ctx context.Context, queryContext table.QueryContext) ([]map[string]string, error) {
