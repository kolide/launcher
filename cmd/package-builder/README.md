...
This section can be skipped if `kolide/launcher` and `kolide/osqueryd` have been set up otherwise you need to set up these TUF repositories. This process is explained in the remainder of this section. The initial set up for Notary should be completed and is beyond the scope of this document. Notary usage information can be found [here](https://github.com/kolide/updater) if needed. If Notary is already set up you're ready to set up repositories for Osqueryd and Locator.  The first step is to  select strong pass phrases and assign them to the following environment variables:
```
NOTARY_DELEGATION_PASSPHRASE=<secret>
