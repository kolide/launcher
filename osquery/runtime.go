...
func LaunchOsqueryInstance(path string) (*OsqueryInstance, error) {
	...
	if _, err := os.Stat(path); err != nil && os.IsNotExist(err) {
		return nil, errors.Wrapf(err, "path not found: %s", path)
	}
...
// IsHealthy will check to determine whether or not the osquery process that is
// being managed by the current instantiation of this OsqueryInstance is
// healthy.
func (o *OsqueryInstance) IsHealthy() error {
