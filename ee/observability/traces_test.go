...
func Test_buildAttributes(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			keyVals:        []any{"event_ts", []int64{time.Date(2023, 6, 4, 1, 1, 1, 0, time.UTC).Unix(), time.Date(2023, 6, 5, 1, 1, 1, 0, time.UTC).Unix()}},
			filename:       "some/path/to/intpkg/int.go",
