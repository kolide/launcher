...
func CertTrustSettingsTablePlugin(flags types.Flags, slogger *slog.Logger) *table.Plugin {
	...
	return tablewrapper.New(flags, slogger, c.name, columns, c.generate,
		tablewrapper.WithDescription("macOS certificate trust settings from `security dump-trust-settings`, flattened as key-value pairs. Supports system and admin domain constraints. Requires a WHERE domain = constraint. Useful for auditing custom certificate trust overrides."),
