...
// ResolveTarget finds the target path and metadata for the given target name, platform,
// arch, and version or channel. versionOrChannel is either a channel ("stable", "beta",
// etc.) or a specific version ("1.2.3").
// Returns the full TUF target path (e.g. "launcher/darwin/universal/launcher-1.2.3.tar.gz").
func ResolveTarget(c *client.Client, targetName, platform, arch, versionOrChannel string) (targetPath string, metadata data.TargetFileMeta, err error) {
	...
	if isChannel {
		return resolveTargetForChannel(targets, targetName, platform, tufArch, versionOrChannel)
	}
	return resolveTargetForVersion(targets, targetName, platform, tufArch, versionOrChannel)
}

func resolveTargetForChannel(targets data.TargetFiles, targetName, platform, arch, channel string) (string, data.TargetFileMeta, error) {
	releaseFile := path.Join(targetName, platform, arch, channel, "release.json")
	releaseTarget, ok := targets[releaseFile]
	if !ok {
...
func resolveTargetForVersion(targets data.TargetFiles, targetName, platform, arch, version string) (string, data.TargetFileMeta, error) {
