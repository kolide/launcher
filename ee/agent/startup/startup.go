...
func GetStartupValue(ctx context.Context, rootDirectory string, flagKey string) (string, error) {
	store, err := agentsqlite.NewStore(ctx, rootDirectory, agentsqlite.TableKeyValuePairs)
	if err != nil {
...
func NewStartupDatabase(ctx context.Context, knapsack types.Knapsack) (*startupDatabase, error) {
	store, err := agentsqlite.NewStore(ctx, knapsack.RootDirectory(), agentsqlite.TableKeyValuePairs)
	if err != nil {
